...
if test ${XK} -eq 0; then
	...
	touch "$T/fkeys.txt"
	parallel --gnu --no-notice --jobs ${N_DOWNLOAD_PARALLEL} --max-args ${N_KEYS_SERIAL} --arg-file "$T/lkeys.txt" --line-buffer --tagstring "{#}" kx_findkeys_batch "$T/fkeys.txt" "{}"
...
grep ^kx_verify "${KX_BASE}/download_all_packages.sh" | sed -e 's/^kx_verify //' | envsubst | \
	parallel --gnu --no-notice --max-args ${N_SIGNATURES_SERIAL}  --line-buffer --tagstring "{#}" kx_verify_batch "$T/verify.txt" "{}"
